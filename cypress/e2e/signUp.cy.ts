...
describe("Sign Up Test", () => {
  beforeEach(() => {
    cy.visit("https://techaway.phungnnl.dev/register");
  });
  ...
  it("should show errors for different password validation scenarios-8 characters, 1 uppercase letter, 1 lowercase letter, and 1 number", () => {
    ...
    cy.get("#confirmPassword-helper-text").should("be.visible").and("contain", "Passwords do not match");
    ...
    cy.get("#password-helper-text").should("be.visible").and("contain", "Password must be at least 8 characters");
    ...
    // Assert that the password length validation error is displayed
    cy.get("#password-helper-text").should("be.visible").and("contain", "Password must contain at least 1 number");
    // Reset the password fields
    cy.get("#password").clear();
    cy.get("#confirmPassword").clear();
